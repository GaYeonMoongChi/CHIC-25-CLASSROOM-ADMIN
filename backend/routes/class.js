...
router.post("/", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "강의가 추가되었습니다.", class: newClass });
  } catch (error) {
...
router.put("/:class_idx", async (req, res) => {
  try {
    console.log(`/api/class/${req.params.class_idx} PUT 요청 받음`, req.body);
    ...
    if (classroom_idx !== undefined)
      existingClass.classroom_idx = classroom_idx;
    ...
    if (class_daytime !== undefined)
      existingClass.class_daytime = class_daytime;
...
router.delete("/:class_idx", async (req, res) => {
  try {
    ...
    if (!existingClass) {
      return res
        .status(404)
        .json({ error: "삭제할 강의 정보를 찾을 수 없습니다." });
    }
