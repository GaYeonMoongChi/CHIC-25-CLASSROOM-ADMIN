...
router.post('/send-code', async (req, res) => {
  try {
    ...
    if (!email.endsWith('@kw.ac.kr')) {
      return res.status(400).json({ error: '광운대학교 이메일(@kw.ac.kr)만 사용 가능합니다.' });
    }
...
router.post('/verify-code', async (req, res) => {
  try {
    ...
    // 저장된 인증 코드가 없는 경우
    if (!storedData) {
      return res.status(400).json({ error: '인증 코드가 만료되었거나 요청되지 않았습니다.' });
    }
...
router.post('/manager/register', async (req, res) => {
  try {
    ...
    if (!['class_admin', 'ad_admin'].includes(type)) {
      return res.status(400).json({ error: '유효한 관리자 유형을 선택하세요.' });
    }
    ...
    res.status(201).json({ message: '관리자 등록 완료', manager: { name, email, type } });
  } catch (error) {
